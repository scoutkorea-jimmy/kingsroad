...
- **관리자 탭 (한켠 숙소 → 한켠 예약, v00.275 개편)**: ① **대시보드**(오늘 체크인·아웃 + 객실 상태, 최상단) ② **숙소 관리**(숙소정보) ③ **객실 관리** ④ **프로모션**(회원가/회원할인 % + 시즌·공휴일·요일 규칙 + 쿠폰 + 연박할인) ⑤ **예약현황**(날짜별 판매중지·재고·요금 오버라이드 캘린더) ⑥ **예약**(상태 색상 표 — 예약대기/확정/체크인/아웃/취소/노쇼, 행 클릭 상세) ⑦ **고객**(VIP·블랙리스트·방문, 1년 보관 단서) ⑧ **결제**(무통장·현장, 정산요약).
- **회원가 (v00.275)**: site_content_kv.hangyeon.memberDiscount(%) — 로그인 회원 예약 시 워커 견적이 자동 적용·표시. 프로모션 탭에서 객실별 정상가/회원가 표로 확인.
- **예약 주체**: 회원가입 후 예약 권장(회원가 적용) + **비회원도 예약 가능**(user 선택적). 비회원에겐 모달에서 회원가입 유도.
- **개인정보**: 수집한 고객 개인정보는 **1년간 보관 후 파기** (예약 동의문 + 고객 탭 명시).
