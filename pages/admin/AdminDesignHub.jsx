...
const ADMIN_VERSION_HISTORY = [
  ...
  },
  {
    ...
    summary: "🏠 전주한켠 — 객실선택 복원(순서: 객실선택→기간상품→숙소소개) + 상품명/가격 표기 조정",
    details: [
      "🛏 [객실선택 복원] 기간 상품으로 교체하며 빠졌던 '객실선택'(숙박 단위 예약) 섹션·상단 날짜/인원 바·예약 모달 복원. 탭/섹션 순서: 객실선택 → 기간 상품 → 숙소소개 → 위치/교통 → 시설 → 이용안내.",
      "🏷 [상품명] 1주일 살아보기 / 2주일 살아보기 / 1달 살아보기.",
      "💰 [가격 표기] 190,000원 ~ / 350,000원 ~ / 550,000원 ~ (시작가 표기, 카드·문의 모달·메일 요약 일괄).",
      "✅ [검증] 빌드·check-syntax·헤드리스 /hangyeon 객실선택+기간상품 동시 렌더, 탭 순서, 문의 모달 정상.",
      "📦 cache-buster — `?v=00.288.001`."
    ]
  },
  {
    version: "00.288.000",
  ...
  },
  {
    ...
    summary: "🛡️ 한켠 갤러리 무한 렌더 위험 차단(사진 0장 케이스) — 안정성 점검",
    details: [
      "🐞 [무한 렌더 fix] v00.284.003 갤러리 회전 로직이 부모의 매-렌더 새 배열 참조(특히 사진 0장 시 `[]` 리터럴)에 흔들려 reset effect→setSide 가 연쇄 발화, '최대 업데이트 깊이 초과' 크래시 가능. pool 을 이미지 url 시그니처 기준으로만 memo 화해 차단(빈 배열 안전).",
      "✅ [라이브 스모크] 워커 /health 200, room-types 신규 weekly/monthly 필드 정상, day 쿼리 가용성/정액 정상, 주간(7박)/월간(30박) 견적·종료일 자동 정상, 시간제-전용 객실 주간 거부 정상, 시간제 최소 3시간=15,000원 정상 검증.",
      "📦 cache-buster — `?v=00.284.004`."
    ],
    context: "사용자 '서비스 안정적으로 되는지 검토' 요청 중 발견·수정. 사진 미등록 숙소에서 예약 페이지 크래시 가능성 제거."
  },
  {
    version: "00.284.003",
    date: "2026-06-06",
    datetime: "2026-06-06T16:10:00+09:00",
    summary: "🎞️ 갤러리 3초 슬라이드 + 시간제 최소시간 기본가 + 기준인원·소스맵 콘솔 정리",
    details: [
      "🎞️ [갤러리] 작은 4장을 3초마다 슬라이드(옆에서 슬라이드-인)로 교체. 직전 4장과 겹치지 않게 무작위로 뽑아 같은 사진이 연속 노출되지 않음(풀이 8장↑일 때 완전 교체).",
      "⏱️ [시간제 카드 가격] 한켠 작업실 등 시간제 객실의 대표가를 '시간당 단가'가 아니라 '최소 이용시간 × 시간당'(예: 5,000×3=15,000원~ 최소 3시간)으로 표시. 실제 최소 결제액과 일치.",
      "👥 [기준 인원] 객실 카드 '기준 N명'이 2로 하드코딩돼 최대 1명 객실에서 '기준 2 / 최대 1' 모순. 기준 = min(2, 최대인원)으로 보정(1인실은 기준 1 / 최대 1).",
      "🧹 [콘솔] CSP connect-src 에 jsdelivr/unpkg 추가 — DOMPurify 소스맵(.map) fetch 차단 경고 2건 제거(DevTools 전용 잡음).",
      "📦 cache-buster — `?v=00.284.003`."
    ],
    context: "사용자 연속 요청: 갤러리 3초 슬라이드·겹침 없음, 시간제 최소시간 기본가, 기준인원 모순, 소스맵 콘솔 경고."
  },
  {
    version: "00.284.002",
    date: "2026-06-06",
