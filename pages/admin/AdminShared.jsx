...
// ─────────────────────────────────────────────────────────────────
// v00.298 — 강연·투어 목록 정렬/필터 (사용자 요청 '목록에서도 유연하게 볼 수 있게').
// 두 패널이 거의 같은 목록이라 판정 로직과 UI 를 여기서 공유한다.
// 회원 패널의 검색 + select 패턴을 그대로 따랐다 — 관리자가 새로 배울 것이 없게.
...
// 순수 함수 — 화면과 분리해 둔다(스모크 테스트로 검증 가능).
// countOf(item) → 유효 신청 수, seatsOf(item) → 잔여석. 없으면 0 으로 본다.
const filterSortEvents = (items, { search = '', status = 'all', sort = 'date-desc', countOf, seatsOf } = {}) => {
  const now = Date.now();
  ...
  const byDate = (a, b, dir) => {
    ...
    // 일정 미정(0)은 방향과 무관하게 항상 맨 뒤 — 위에 뜨면 목록이 쓸모없어진다.
...
const EventListToolbar = ({ search, onSearch, status, onStatus, sort, onSort, shown, total, placeholder }) => (
  <div style={{display:'flex', gap:10, marginBottom:16, alignItems:'center', flexWrap:'wrap'}}>
    ...
    <select className="field-input" style={{maxWidth:130}} value={status} onChange={(e) => onStatus(e.target.value)}
      aria-label="상태 필터">
      {EVENT_FILTERS.map((f) => <option key={f.key} value={f.key}>{f.label}</option>)}
    </select>
    <select className="field-input" style={{maxWidth:160}} value={sort} onChange={(e) => onSort(e.target.value)}
      aria-label="정렬 기준">
