...
- PG 결제 (현재 무통장 임시 방편)
- ⚠️ 환불 신청 흐름 — 확정(입금 완료) 상태에서 취소 시 실제 환불 요청 경로 없음. 책과 동일한 패턴 적용 예정.
- D-1 알림 · 변경 알림 (이메일 / 푸시)
- 참가자 명단 · 체크인 · 출석 이력
...
- ⚠️ 환불 신청 흐름 미구현 — 취소 후 실제 환불은 운영자가 수동 처리. 책과 동일한 `requestRefund/approveRefund/rejectRefund` 패턴 적용 예정.
...
3. **강연/투어 환불 신청 흐름** — ⚠️ P5에서 책만 구현됨. 강연/투어도 동일한 requestRefund 패턴 적용 필요
4. **이메일 알림 인프라** — hook 먼저 추가, 발송은 비활성화. Cloudflare Email Workers 사용 예정
5. **마이페이지 프로필 수정 / 비밀번호 변경** — Cloudflare 인증 전환 후 착수

✅ 완료 (P5):
- 환불 처리 흐름 — 책 주문 환불 신청 + 관리자 승인/반려 사이클 구현 (강연/투어는 별도 착수 예정)
