...
- 환불 흐름이 없어 취소 후 실제 환불은 운영자가 수동 처리해야 함
...
1. **외부 DB + 서버 인증 전환** — 현재 local-first 구조가 실 서비스의 가장 큰 한계. 다른 모든 기능의 전제.
2. **PG 결제 연동** — 강연·투어·책 모두 무통장 임시 방편 상태
3. **이미지 외부 스토리지** — localStorage quota 문제(이미지 base64 저장)
4. **이메일 알림** — D-1 강연/투어 알림, 상태 변경 이메일

✅ 완료 (P5):
- 환불 처리 흐름 — 사용자 환불 신청 + 관리자 승인/반려 사이클 구현
